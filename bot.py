...
def get_lol_data_from_sheet():
    ...
    try:
        ...
        tier_scores = {item['키']: item['값'] for item in settings_data if item['설정명'] == '티어점수'}
        position_weights = {item['키']: item['값'] for item in settings_data if item['설정명'] == '포지션가중치'}
...
def balance_teams(player_names, tier_scores, position_weights, player_db):
    ...
    # 2. 개인별 모든 포지션의 환산점수 미리 계산
    score_matrix = {}
    for player in participants:
        player_name = player['이름']
        tier_score = tier_scores.get(player['티어'], 0)
